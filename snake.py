...
def main():
    ...
    for session in tqdm(range(args.sessions)):
        ...
        if args.visual:
            ...
            w = tk.Scale(
                root, from_=10, to=500,
                label="Refresh Rate (ms)", orient=tk.HORIZONTAL)
            w.set(refresh_rate)
            w.grid(row=environment.board.height, column=0,
                   columnspan=environment.board.width, sticky="ew")
            ...
            for row in range(environment.board.height):
                for col in range(environment.board.width):
                    cell = tk.Label(root, width=6, height=3,
                                    borderwidth=1, relief="sunken", bg="black")
            ...
            def update_display():
                ...
                # Draw red apple
                red_cell = cells[(environment.board.red_apple[1],
                                  environment.board.red_apple[0])]
                ...
                for snake_part in environment.snake.body:
                    ...
                    snake_cell.config(bg="blue" if snake_part !=
                                      environment.snake.body[0] else "yellow")
                ...
                nonlocal duration
                duration += 1
                root.after(w.get(), update_display)
